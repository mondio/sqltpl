...
/**
 * kj_master_occupation
 * @author hezhe
 */
public class kj_master_occupation {
	public static void satrt(Integer seqId,String update_per_code,String contract_company_id){
		String parentName="kj_master_occupation_group";
		String childName="kj_master_occupation";
		String parentSeq="nextval('kj_master_occupation_group_seq')";
		String childSeq="nextval('kj_master_occupation_seq')";
		...
		for (String line : resultChlidList) {
			if(StrUtil.isNotBlank(line) && line.toUpperCase().contains("VALUES (")){
				...
				String[] values =line.substring(line.indexOf("VALUES (")+8,line.lastIndexOf(")")).split("', '");
				values[0]=childSeq;
				String newId=seqMap.get(Util.replaceFEchar(values[3]));
